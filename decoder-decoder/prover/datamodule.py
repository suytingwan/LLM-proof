...
def read_ruletaker_proofs(path: str, is_train: bool) -> List[Example]:
    ...
    for line in enumerate(open(path)):
        ex = json.loads(line)
        hypothesis = normalize(ex["hypothesis"])
        context = extract_context(ex["context"])
...
class EntireProofsDataset(Dataset):  # type: ignore
    ...
    def collate(self, examples: List[Example]) -> Batch:
        ...
        input_attention_mask = input_seq.attention_mask
        output_seq_mask = input_attention_mask.clone()
        for i in range(inp_exp_seq.attention_mask.shape[0]):
            labels[i, :sum(inp_exp_seq.attention_mask[i])] = -100 #ignore index
        ...
        batch = {
            ...
            "output_seq": oup,
            "output_seq_ids": labels,
            "output_seq_mask": output_seq_mask
        }
...
class StepwiseDataset(Dataset):  # type: ignore
    ...
    def collate(self, examples: List[Example]) -> Batch:
        ...
        if self.is_train:
            ...
            input_seq_mask = input_seq.attention_mask
            output_seq_mask = input_seq_mask.clone()
            for i in range(inp_exp_seq.attention_mask.shape[0]):
                labels[i, :sum(inp_exp_seq.attention_mask[i])] = -100 #ignore index
                output_seq_mask[i, :sum(inp_exp_seq.attention_mask[i])] = 0

            batch["output_seq"] = oup
            batch["output_seq_ids"] = labels
            batch["output_seq_mask"] = output_seq_mask
        return batch
...
if __name__ == "__main__":
    ...
    pre_train = "/home/ysuay/codes/LLM+Reason/codes/NLProofS/data/proofwriter-dataset-V2020.12.3/preprocessed_OWA/depth-3ext/meta-train.jsonl"
    pre_valid = "/home/ysuay/codes/LLM+Reason/codes/NLProofS/data/proofwriter-dataset-V2020.12.3/preprocessed_OWA/depth-3ext/meta-dev.jsonl"
    pre_test = "/home/ysuay/codes/LLM+Reason/codes/NLProofS/data/proofwriter-dataset-V2020.12.3/preprocessed_OWA/depth-3ext/meta-test.jsonl"
    ...
    print(len(ds_train))
    print(ds_train[0])
    ds_train.collate([ds_train[0], ds_train[1]])
