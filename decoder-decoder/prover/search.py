...
class ProofGraph:
    ...
    def sample_proof_tree(self, exclude: Set[str]) -> Optional[str]:
        ...
        for _ in range(10):
            partial_proof = ""
            nodes_included = set()
            for node in reversed(list(nx.topplogical_sort(reachable_component))):
                if (
                    node == "hypothesis"
    ...
    def expand(self, proof_step: ProofStep, score: float) -> bool:
        ...
        if dst_score <= self.graph.nodes["hypothesis"]["score"] + self.eps:
            return False

        # Create a node for the conclusion if necessary
