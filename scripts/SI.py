...
Reasoning with selection stage and inference stage, and finally output the proof until the hypothesis is proved:
step1: (selection) sent13: a bird is a kind of animal. We know that an animal is a kind of organism (sent24);
step1: (inference) from sent13 & sent24, we can infer that a bird is a kind of organism (int1).
step2: (selection) sent2: if a fossil is of an organism that cannot be identified then that organism is probably extinct. We know that a bird is a kind of organism (int1);
step2: (inference) from int1 & sent2, we can infer that if a fossil of a bird cannot be identified then that kind of bird is probably extinct (hypothesis);
...
step2: (inference) from int1 & sent2, we can infer that an animal requires water and air for survival (int2).
step3: (selection) sent13: an animal / living thing requires nutrients for survival. We know that an animal needs to eat food for nutrients (sent21);
step3: (inference) from sent13 & sent21, we can infer that an animal requires food for survival (int3).
step4: (inference) from int2 & int3, we can infer that an animal requires water and air and food for survival (hypothesis).
Combining all the inference steps, output the final proof as:
...
Reasoning with selection stage and inference stage, and finally output the proof until the hypothesis is proved:
step1: (selection) sent19: blue is a kind of energy. We know that blue stars are the hottest in temperature (sent5);
step1: (inference) from sent19 & sent5, we can infer that stars that are blue in color are hottest in temperature (hypothesis).
Combining all the inference steps, output the final proof as:
...
step2: (selection) sent17: water is a kind of substance. sent4: if a substance absorbs solar energy then that substance will increase in temperature. We know that the water in the bucket will absorb solar energy (int1);
step2: (inference) from int1 & sent17 & sent4, we can infer that the water in the bucket will increase in temperature (int2).
step3: (selection) sent17: water is a kind of substance. sent6: evaporation means a substance changes from a liquid into a gas by increasing heat energy. We know that  water in the bucket will increase in temperature (int2);
step3: (inference) from int2 & sent17 & sent6, we can infer that the water in the bucket will evaporate (hypothesis);
Combining all the inference steps, output the final proof as:
...
Reasoning with selection stage and inference stage, and finally output the proof until the hypothesis is proved:
step1: (selection) sent16: a sperm cell is a kind of sex cell. sent17: a mutation in the sex cells of a parent can cause a new trait to appear in the parent 's offspring. We know that an egg cell is a kind of sex cell (sent18);
step1: (inference) from sent16 & sent17 & sent18, we can infer that a mutation in sperm or egg of a parent can cause a new trait to appear in the parent 's offspring (hypothesis).
Combining all the inference steps, output the final proof as:
...
step4: (selection) sent4: moisture is a form of water. We know that a lack of water prevents the survival of plants in the desert (int3);
step4: (inference) from int3 & sent4, we can infer that a lack of moisture prevents the survival of plants in the desert (hypothesis).
Combining all the inference steps, output the final proof as:
...
Reasoning with selection stage and inference stage, and finally output the proof until the hypothesis is proved:
step1: (selection) sent22: a storm is a source of precipitation. We know that rain is a kind of precipitation (sent8);
step1: (inference) from sent22 & sent8, we can infer that a storm is a source of rain (int1).
step2: (selection) sent13: a thunderstorm is a kind of storm. We know that a storm is a source of rain (int1);
step2: (inference) from int1 & sent13, we can infer that a thunderstorm is a source of rain (int2).
step3: (selection) sent9: cold fronts cause thunderstorms as they pass by. We know that a thunderstorm is a source of rain (int2);
step3: (inference) from int2 & sent9, we can infer that cold fronts can cause thunderstorms of rain as they pass by (hypothesis).
Combining all the inference steps, output the final proof as:
...
Reasoning with selection stage and inference stage, and finally output the proof until the hypothesis is proved:
step1: (selection) sent18: a heterozygous organism contains one dominant gene and one recessive gene. We know that a tay-sachs disease carrier have one dominant and one recessive tay-sachs gene (sent22);
step1: (inference) from sent18 & sent22, we can infer that a tay-sachs disease carrier is heterozygous (int1).
step2: (selection) sent23: crossing two heterozygous dominant organisms causes their offspring to be homozygous recessive of 25% probability. We know that a tay-sachs disease carrier is heterozygous (int1);
step2: (inference) from int1 & sent23, we can infer that the offspring of two tay-sachs carriers will have 25% probability to be homozygous recessive in tay-sachs disease (int2).
step3: (selection) sent25: a homozygous recessive organism contains only recessive genes. We know that the offspring of two tay-sachs carriers will have 25% probability to be homozygous recessive in tay-sachs disease (int2);
step3: (inference) from int2 & sent25, we can infer that the offspring of two tay-sachs carries will have 25% probability to contain only recessive genes for tay-sachs disease (int3).
step4: (selection) sent1: if only recessive genes are present , then the recessive trait will be visible / expressed. We know that the offspring of two tay-sachs carries will have 25% probability to contain only recessive genes for tay-sachs disease (int3);
step4: (inference) from int3 & sent1, we can infer that the offspring of two tay-sachs carriers will have 25% probability to have the tay-sachs disease (hypothesis);
Combining all the inference steps, output the final proof as:
...
Reasoning with selection stage and inference stage, and finally output the proof until the hypothesis is proved:
step1: [selection] sent17: sulfur cannot be decomposed into different substances by simple chemical methods. We know that an element cannot be decomposed into two or more different substances by simple chemical methods (sent25);
step1: [inference] from sent17 & sent25, we can infer that sulfur is a kind of element (hypothesis).
Combining all the inference steps, output the final proof as:
...
step3: [inference] from sent10 & sent2, we can infer that volvox is a kind of protist that reproduces sexually (int3).
step4: [inference] from int2 & int3, we can infer that the protist may be volvox (hypothesis).
Combining all the [inference] steps, output the final proof as:
