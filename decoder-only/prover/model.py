...
class PermutationInvarianceLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        for i, predix in enumerate(generated_texts):
            if "->" in predix: # conclusion
                if prefix.count("->") > 1:
                    scores[i, :] = float("-inf")
                    continue
                ...
                n = i // self.num_beams
                premises = tuple(sorted([p.strip() for p in predix.split("&")]))
...
class EntailmentWriter(pl.LightningModule):
            ...
            for step in proof_gt.proof_steps:
                for sent in step.premise_sents:
                    if sent not in partial_proof:
                        ...
                        premise_idents = []
                        for ident, sent in zip(step.premises_idents, step.premise_sents):
                            if re.fullmatch(r"int\d+", ident):
                                ident = re.search(f"(?P<ident>int\d+): {sent}", inp)[
                                    "ident"
                                ]
                            premises_idents.append(ident)
    ...
    def search_proof(
        ...
        pg = ProofGraph(context, hypotheis)
